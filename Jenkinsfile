...
pipeline {
  ...
  environment {
     APP_NAME = 'BuyRequest/product-service'
     WORKSPACE = '/home/Jenkins/workspace/BuyRequest/product-service'
  }
   agent none
   stages {
            ...
            steps {
                script {
                    sshagent (credentials: ['buyrequestdev']) {
                        ...
                        && cat .env \
                        && cd ../services/product-service && git checkout -f && git pull \
                        && cd /home/ubuntu/buy-request/docker && make product-service-restart && make nginx-restart \
                        && make product-service-logs-without-tail \
